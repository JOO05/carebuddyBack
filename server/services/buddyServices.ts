...
class BuddyyService {
  ...
  // 반려동물 정보 수정 및 삭제
  async updateAndDeleteBuddy(_id: mongoose.Types.ObjectId, checkBuddy: checkBuddy) {
    let updateData: Partial<checkBuddy> = checkBuddy;

    if (checkBuddy.isNeutered !== undefined) { // isNeutered가 변경된 경우에만 updatedAt 필드를 업데이트
      updateData.isNeutered = new Date();
    }

    const foundBuddy = await this.buddyModel.updateBuddy(_id, updateData);
    if (!foundBuddy) return { status: 404, err: '작업에 필요한 반려동물이 없습니다.' }
